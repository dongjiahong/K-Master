...
- **📊 双周期同屏分析 (Dual-Timeframe)**
  ...
  - 左侧显示大周期（HTF，如 4 小时/日线）提供趋势背景。
  - 右侧显示小周期（LTF，如 5 分钟/15 分钟）进行入场操作。
  - 数据实时同步，完美模拟真实看盘体验。

- **🤖 AI 智能教练 (Gemini Powered)**

  - **入场分析**：点击 AI 分析前需填写交易计划，AI 会根据 K 线数据分析入场逻辑、盈亏比和趋势一致性。
  - **交易复盘**：无论自动还是手动止盈止损，AI 都会自动进行平仓复盘分析。
  - **终局报告**：游戏结束时生成幽默犀利的 Markdown 格式总结报告（按交易时间正序分析）。
  - **K 线数据分析**：AI 直接分析小周期和大周期 K 线数据，识别形态和趋势。
...
- **📊 生涯统计系统**
  ...
  - **趋势图表**：折线图直观展示每局利润率和胜率的变化趋势。
  - **胜率追踪**：每局游戏记录显示交易笔数和胜率百分比。
  - **对比分析**：同场景多次尝试的成绩横向对比。
...
2.  **配置环境**
    ...
    _(注：在当前演示代码结构中，`process.env.API_KEY` 需要构建工具注入)_
